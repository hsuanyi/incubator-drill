...
public class DrillPushProjectPastWindowRule extends RelOptRule {
  public static final DrillPushProjectPastWindowRule INSTANCE = new DrillPushProjectPastWindowRule();

  private DrillPushProjectPastWindowRule() {
    super(
        operand(
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    final List<RexNode> exps = Lists.newArrayList();
    final List<RelDataTypeField> fields = Lists.newArrayList();

    // Keep only the fields which are referred
    for(int index = beReferred.nextSetBit(0); index >= 0; index = beReferred.nextSetBit(index + 1)) {
    ...
    final List<RelDataTypeField> outputWindFields = Lists.newArrayList(inputWinFields);
    ...
    final List<RexNode> topProjExps = Lists.newArrayList();
    ...
    if (ProjectRemoveRule.isTrivial(newTopProj, true)) {
