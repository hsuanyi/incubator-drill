...
public class ScreenPrel extends DrillScreenRelBase implements Prel {
  ...
  private Map<String, TypeProtos.MajorType> schemaInPlanning;
    ...
    if (AssertionUtil.isAssertionsEnabled()) {
      ...
      for (RelDataTypeField relDataTypeField : fieldList) {
        ...
        final TypeProtos.MinorType minorType =
            TypeInferenceUtils.getDrillTypeFromCalciteType(relDataTypeField.getType());
        final TypeProtos.MajorType majorType =
            relDataTypeField.getType().isNullable() ? Types.optional(minorType) : Types.required(minorType);
        schemaInPlanning.put(relDataTypeField.getName(), majorType);
      }
    }
  }
