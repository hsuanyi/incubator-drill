...
public abstract class AbstractSchema implements Schema, SchemaPartitionExplorer, AutoCloseable {
  ...
  /**
   * Get the collection of {@link Table} tables specified in the tableNames. It is not guaranteed that the retrieved tables
   * would have RowType and Statistic being fully populated.
   *
  ...
  public List<Pair<String, ? extends Table>> getTablesByNames(final List<String> tableNames) {
    return getFullyPopulatedTablesByNames(tableNames);
  }
   ...
   * @return the collection of requested tables
   */
  public List<Pair<String, ? extends Table>> getFullyPopulatedTablesByNames(final List<String> tableNames) {
